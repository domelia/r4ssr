...
### 
Хотя в рамках лекции мы обсуждали, что библиотека `ggplot2` не поддерживает пайпинг внутри кода, создающего график, ничто не мешает нам с помощью пайпа присоединить функции ggplot2 к самим данным. Такой способ представления аналогичен конструкции `ggplot(data = penguins). 
...
### Exercise 10
В настощее время наш график показывает одну линию для всех данных, вместо того, чтобы показать отдельные линии для каждого вида пингвина. Чтобы создать такой эффект, нужно удалить настройки цвета -  `color` внутри функции`ggplot()` и добавить их в слой `geom_point()`: `geom_point(mapping = aes(color = species))`.
Скипируйте код из прерыдущего упражнения и поменяйте настройки цвета
...
```{r first-steps-12-solution, include = FALSE}
penguins |>
  drop_na() |>
  ggplot(mapping = aes(x = flipper_length_mm, 
                       y = body_mass_g)) +
...
```{r first-steps-14-solution, include = FALSE}
penguins |>
  drop_na() |>
  ggplot(mapping = aes(x = flipper_length_mm, 
                       y = body_mass_g)) +
